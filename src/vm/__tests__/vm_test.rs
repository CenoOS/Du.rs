...
mod tests {
    ...
    fn should_jmp() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 0, 1 /*LOAD 0 #1; */, 6, 0]; /*JMP 0; */
        vm.run_once();
        vm.run_once();
        assert_eq!(vm.pc, 1);
    }
    ...
    fn should_jmpf() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 0, 3 /*LOAD 0 #3; */, 7, 0]; /*JMPF 0; */
        vm.run_once();
        assert_eq!(vm.pc, 4);
        vm.run_once();
        assert_eq!(vm.pc, 9);
    ...
    fn should_jmpb() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 0, 3 /*LOAD 0 #3; */, 8, 0]; /*JMPB 0; */
        vm.run_once();
        assert_eq!(vm.pc, 4);
        vm.run_once();
        assert_eq!(vm.pc, 3);
    ...
    fn should_inc() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 1, 244 /*LOAD 0 #500; */, 12, 0]; /*INC $0; */
    ...
    fn should_dec() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 1, 244 /*LOAD 0 #500; */, 13, 0]; /*DEC $0; */
    ...
    fn should_opcode_and() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 0, 3, 1, 1, 0, 7, 33, 0, 1, 2];
        vm.run();
    ...
    fn should_opcode_or() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 0, 3, 1, 1, 0, 7, 34, 0, 1, 2];
        vm.run();
        assert_eq!(vm.registers[2], 3 | 7);
    }
    ...
    fn should_opcode_xor() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 0, 3, 1, 1, 0, 7, 35, 0, 1, 2];
        vm.run();
        assert_eq!(vm.registers[2], 3 ^ 7);
    }
    ...
    fn should_aloc() {
        let mut vm = VM::new();
        vm.program = vec![1, 0, 1, 244 /*LOAD 0 #500; */, 11, 0]; /*ALOC $0; */
    ...
    fn should_loop_add_and_print() {
        let mut vm = VM::new();
        vm.ro_data = vec![
            72, 101, 108, 108, 111, 44, 32, 87, 111, 114, 108, 100, 46, 0, 79, 107, 44, 32, 53, 48,
            32, 116, 105, 109, 101, 115, 32, 112, 114, 105, 110, 116, 32, 112, 97, 115, 115, 101,
            100, 46, 0,
        ];
        vm.program = vec![
            ...
            0x0C, 0x0F, 0x1F, 0x01, 0x1F, 0x00, 0x0E, 0x0E, 0x1F,
        ];
        vm.run();
        assert_eq!(vm.pc, 37);
        assert_eq!(vm.registers[2], 51);
